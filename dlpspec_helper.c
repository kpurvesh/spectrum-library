...
void dlpspec_copy_chemoScanData_hdr_to_scanResults(const chemoScanData *pScanData,scanResults *pResults)
{
	pResults->header_version = pScanData->header_version;
	pResults->year = pScanData->year;
	pResults->month = pScanData->month;
	pResults->day = pScanData->day;
	pResults->day_of_week = pScanData->day_of_week;
	pResults->hour = pScanData->hour;
	pResults->minute = pScanData->minute;
	pResults->second = pScanData->second;
	pResults->system_temp_hundredths = pScanData->system_temp_hundredths;
	pResults->detector_temp_hundredths = pScanData->detector_temp_hundredths;
	pResults->humidity_hundredths = pScanData->humidity_hundredths;
	pResults->lamp_pd = pScanData->lamp_pd;
	memcpy(&(pResults->calibration_coeffs), &(pScanData->calibration_coeffs),sizeof(calibCoeffs));
	memcpy(pResults->serial_number, pScanData->serial_number, NANO_SER_NUM_LEN);
	memcpy(pResults->scan_name, pScanData->scan_name, SCAN_NAME_LEN);
	pResults->scanDataIndex = pScanData->scanDataIndex;
	pResults->pga = pScanData->pga;
	memcpy(&pResults->chemoCfg, &pScanData->chemoCfg, sizeof(chemoScanConfig));
}
...
bool dlpspec_is_chemodatatype(void *struct_p, size_t buffer_size)
{
	char *fmt;
	bool ret;
    char std_cfg_format[] = "S(" SCAN_DATA_FORMAT ")";
...
void dlpspec_chemo_subtract_remove_dc_level(const chemoScanData *pScanData,
		scanResults *pResults)
/**
 * Subtracts the magnitude of the detector values stored during off patterns from
 * the detector values for all other patterns of the scan. This is used while
